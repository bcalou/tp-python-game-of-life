...
class Grid:
    ...
    def update_grid(self):
        ...
        __grid_len_row = len(self.matrix)
        __grid_len_col = len(self.matrix[0])

        # Grille de l'état suivant
        __next_state = [[0] * __grid_len_col for _ in range(__grid_len_row)]
            ...
            for col_id in range(__grid_len_col):
                ...
                if self.matrix[row_id][col_id] == 1:  # Si cellule vivante
                    if live_neighbors < 2 or live_neighbors > 3:
                        # Mort par sous-population ou surpopulation
                        __next_state[row_id][col_id] = 0
                    else:
                        # Survie
    ...
    def __count_living_neighbors(self, row_id: int, col_id: int) -> int:
        ...
        for x_neighbor, y_neighbor in neighbors_coordinates:
            ...
            if 0 <= row_neighbor < rows_len and 0 <= col_neighbor < cols_len:
                live_neighbors += self.matrix[row_neighbor][col_neighbor]
    ...
    def __add_preset(self, preset: Array2):
        ...
        row_center = (height // CELL_SIZE - preset_len_col) // 2
