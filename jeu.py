...
def count_neighbours(current_state: list[list[int]], x: int, y: int) -> int:
...
def get_next_state(current_state: list[list[int]]) -> list[list[int]]:
    ...
    for y in range(len(current_state)):
        for x in range(len(current_state[y])):
            neighbourCellsCount: int = count_neighbours(current_state, x, y)

            if neighbourCellsCount < 2 or neighbourCellsCount > 3:
                next_state[y][x] = 0
            elif neighbourCellsCount == 3:
                next_state[y][x] = 1
    return next_state